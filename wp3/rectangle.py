...
class Rectangle(Tile):
    def calculate_center(self):
        ...
        x = (self.side_length + self.spacing) * self.col + self.spacing + self.side_length / 2
        y = (self.side_length + self.spacing) * self.row + self.spacing + self.side_length / 2
    ...
    def create_patches(self):
        ...
        inner = RegularPolygon((self.x, self.y), 4, radius=self.side_length/np.sqrt(2), orientation=np.pi/4)
        outer = RegularPolygon((self.x, self.y), 4, radius=(self.side_length+2*self.spacing)/np.sqrt(2), orientation=np.pi/4)
        return inner, outer
    ...
    def contains(self, x, y):
        ...
        return 2 * np.abs(x - self.x) <= self.side_length and 2 * np.abs(y - self.y) <= self.side_length
