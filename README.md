...
- To add new panel types, you need to create a new python file inside `wp3` and give it a meaningful name, *e.g.*, `star.py` if you are creating star-shaped panels. You then have to create inside it a class that inherits from `Tile` and implement the methods `calculate_center()`, `create_patches()`, `vertices()`, `contains()`, `adjacent()` and, optionally, `get_variants()`. Finally, import the tile type into the `wp3` module. The instruction (to be added into `wp3/__init__.py`) might look like `from .start import Star`. The class
